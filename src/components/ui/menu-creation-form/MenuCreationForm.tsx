...
import { createMenu } from '@vat/actions/menu.actions';

import { intNumberRegex } from '@vat/lib/regex';
import { menuFormSchema } from '@vat/lib/schemas';

import { CreateMenuFormProps } from '@vat/types/menu.types';

export const MenuCreationForm = () => {
  const { dispatch } = useSnackbarContext();
  const { setLoading } = useDrawerLoading();

  const router = useRouter();

  const formik = useFormik<CreateMenuFormProps>({
    initialValues: {
      title: '',
      index: '',
    },
  ...
  async function handleSubmit(values: CreateMenuFormProps) {
    try {
      setLoading(true);
  ...
  return (
    <form
      ...
      <div className='flex gap-4 pt-6'>
        ...
        <Button className='w-1/2'>Create menu</Button>
      </div>
    </form>
  );
};
